...
fn basic_answer_a(array: &[u16; 200], target: u16) -> u32 {

    for (index, item1) in array.iter().enumerate() {
        let array_subset: &[u16] = &array[index..array.len()-1];
        for item2 in array_subset.iter() {
            if item1 + item2 == target {
                let answer: u32 = u32::from(*item1)*u32::from(*item2);
                return answer
            }
        }
    }
...
fn smart_answer_a(array: &[u16; 200], target: u16) -> u32 {
    for (index, item1) in array.iter().enumerate() {
        let item2: u16 = target - item1;
        let array_subset: &[u16] = &array[index..array.len()-1];
        let result = array_subset.binary_search(&item2);
        if result.is_ok() {
            let answer: u32 = u32::from(*item1)*u32::from(item2);
            return answer
        }
...
fn answer_b(array: &[u16; 200], target: u16) -> u32 {
    for (index1, item1) in array.iter().enumerate() {
        let array_subset_1: &[u16] = &array[index1..array.len()-1];    
        for (index2, item2) in array_subset_1.iter().enumerate() {
            if target > item1 + item2 {
                let item3: u16 = target - item1 - item2;
                if item3 > *item2 {
                    let array_subset_2: &[u16] = &array[index2..array.len()-1];  
                    let result = array_subset_2.binary_search(&item3);
                    if result.is_ok() {
                        let answer: u32 = u32::from(*item1)*u32::from(*item2)*u32::from(item3);
                        return answer
                    }
                }
            }
